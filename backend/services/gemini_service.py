...
{
    "category": "Produtivo" | "Improdutivo" | {Any of the customCategories in the customCategories parameter} classify the email based on the context of the email if it is necessary to respond or not respectively (productive or unproductive or some custom category, but if the email is productive and fits is some of the custom categories send the custom category that matches the most) is the name of the custom category is too big you can simplify in 2 words first letter of first word in uppercase and the rest in lowercase and the second word in lowercase, and the most important thing is that the category must match with the context of the email, and the most important thing: if the custom categories field are empty, do NOT send any custom category, only send "Produtivo" or "Improdutivo"
